...
- Project scaffolding: `pyproject.toml` publishing the `3dtk` distribution with
  the `py3dtk` import package and a `3dtk` console script, GPLv3 licence.
- `py3dtk.catalog`: the four-step catalogue resolver (`--db` → `PY3DTK_DB`
...
- `py3dtk.sources`: denormalised join SQL for all seven targets, carrying
...
- `py3dtk.query`: the declarative `TargetConfig`/`TARGETS` engine, filter
...
- `py3dtk.output`, `py3dtk.values`, `py3dtk.fields`, `py3dtk.stats`:
...
- `py3dtk.counts`: dense reconstruction of the sparse count matrices. Axes
...
- `py3dtk.terms`: data-usage gate with placeholder wording marked `TODO`
  rather than invented policy.
- `py3dtk.ena`: ENA Portal API resolution of run accessions to FASTQ URLs,
...
- `py3dtk.download`: chunked HTTP/FTP downloads with Rich progress, resumable
  ...
  re-checks MD5s too.
- `py3dtk.fetch`: resolves a filtered record set into download jobs, reporting
...
- `py3dtk.api`: `Database` context manager with seven typed collections, each
  supporting `query`, `count`, `values` and `stats`; `fetch` on macrosamples and
  microsamples; `matrices`, `export` and `coordinates` on counts.
- `py3dtk.filters` + `py3dtk.commands`: CLI filter specifications and the
...
- Repository furniture: `README.md` (documenting the `pip install 3dtk` →
  `import py3dtk` split prominently), `RELEASING.md` with the four version
...
- Package metadata now uses the current SPDX licence declaration, avoiding
  setuptools deprecation warnings during distribution builds.
